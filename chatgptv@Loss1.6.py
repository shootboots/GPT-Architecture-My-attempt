...
if response.status_code == 200:
    text = response.text
    print("Success: file saved to text variable")
else:
...
response = decode(m.generate(torch.tensor([prompt], dtype=torch.long, device=device), max_new_tokens=4000)[0].tolist())


outputFile = open("output.txt", "w")
outputFile.write("Prompt: " + str(decode(prompt)) + "\n" + "\n" + str(response))
